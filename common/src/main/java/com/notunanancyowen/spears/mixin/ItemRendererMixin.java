...
public abstract class ItemRendererMixin {
    ...
    private void renderSpears(ItemRenderer instance, BakedModel model, ItemStack stack, int light, int overlay, MatrixStack matrices, VertexConsumer vertices, Operation<Void> original, @Local(argsOnly = true) ModelTransformationMode renderMode) {
        if(stack.getItem() instanceof SpearItem && (renderMode == ModelTransformationMode.GUI || renderMode == ModelTransformationMode.GROUND || renderMode == ModelTransformationMode.FIXED)) original.call(instance, models.getModel(stack), stack, light, overlay, matrices, vertices);
